...
exports.main = async (event, context) => {
  ...
  try {
   ...
   .orderBy(db.serverDate(), 'asc') // 最早该复习的单词优先
   .get();

 if (reviewList.data.length === 0) {
   return { success: true, data: [] }; // 无待复习单词
 }
 ...
 return {
   ...
   data: {
    ...
    return { success: false, error: '获取复习单词失败' };
  }
};
...
async function getDistracts(targetWord, level) {
    ...
    while (distracts.size < 3) {
      const randomWord = await db.collection(collectionName)
        .skip(Math.floor(Math.random() * await db.collection(collectionName).count()))
        .limit(1)
        .get();
      
      const translate = randomWord.data[0].translate;
      if (translate !== targetWord.translate) {
        distracts.add(translate);
      }
    }
